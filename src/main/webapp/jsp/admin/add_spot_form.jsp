...
<div class="add_spot_popup" id = "add_spot_popup">
  <h1 class = "title" id = "add_label" hidden>Add</h1>
    <form class="spot_form" id="add_spot" onsubmit="return onAddSpot(this)">

        <input id = "uid" name="uid" type="hidden"/>

        <%--<label for="region"><fmt:message bundle="${loc}" key="locale.title"/></label>--%>
        <input class="region" id = "region" type="text" name="region" placeholder="region" required/>

        <%--<label for="city"><fmt:message bundle="${loc}" key="locale.type"/></label>--%>
        <input class="city" id = "city" type="text" name="city" placeholder="city" required/>

        <%--<label for="street"><fmt:message bundle="${loc}" key="locale.season"/></label>--%>
		<input class="street" id = "street" type="text" name="street" placeholder="street" required/>

        <%--<label for="region"><fmt:message bundle="${loc}" key="locale.title"/></label>--%>
        <input class="region" id = "region_ru" type="text" name="region_ru" placeholder="region_ru" required/>

        <%--<label for="city"><fmt:message bundle="${loc}" key="locale.type"/></label>--%>
        <input class="city" id = "city_ru" type="text" name="city_ru" placeholder="city_ru" required/>

        <%--<label for="street"><fmt:message bundle="${loc}" key="locale.season"/></label>--%>
        <input class="street" id = "street_ru" type="text" name="street_ru" placeholder="street_ru" required/>

        <%--<label for="house"><fmt:message bundle="${loc}" key="locale.coast"/></label>--%>
        <input class="house" id = "house" type="text" name="house" placeholder="house" required/>

        <button type="submit" id = "submit_changes" class="btn ok">
            <fmt:message bundle="${loc}" key="locale.submit"/>
        </button>
        <button type="button" id="cancel_changes" class="btn cancel" onclick="closeAddForm()">
